...
namespace OpenTibiaUnity.UI.Legacy
{
    ...
    public class ScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [SerializeField]
        private float m_MinHorizontalScrollbarHeight = 14f;
        ...
        [SerializeField]
        private float m_MinVerticalScrollbarWidth = 14f;
        ...
        private void UpdateScrollbars(Vector2 offset)
        {
            if (m_HorizontalScrollbar)
            {
                ...
                {
                    var totalSliderAreaHeight = (m_HorizontalScrollbar.handleRect.parent as RectTransform).rect.width;
                    var minSize = m_MinHorizontalScrollbarHeight / totalSliderAreaHeight;
                    m_HorizontalScrollbar.size = Mathf.Max(minSize, Mathf.Clamp01((m_ViewBounds.size.x - Mathf.Abs(offset.x)) / m_ContentBounds.size.x));
                }
                else
                {
            ...
            if (m_VerticalScrollbar)
            {
                ...
                {
                    var totalSliderAreaWidth = (m_VerticalScrollbar.handleRect.parent as RectTransform).rect.height;
                    var minSize = m_MinVerticalScrollbarWidth / totalSliderAreaWidth;
                    m_VerticalScrollbar.size = Mathf.Max(minSize, Mathf.Clamp01((m_ViewBounds.size.y - Mathf.Abs(offset.y)) / m_ContentBounds.size.y));
                }
                else
                {
