    ...
    public partial class ProtocolGame : Internal.Protocol
    {
            ...
            switch (messageType)
            {
                ...
                case GameserverMessageType.MoveCreature:
                    try
                    {
                        ...
                        //NewGameManager.inst.RenderGroundAroundMe();
                    }
                    catch (Exception ex)
                    {
                        UnityEngine.Debug.Log("Move creature error");
                    }
        ...
        public void OnCheckAlive()
        {
            ...
            if (IsGameRunning || IsPending)
            {
                ...
                else if (!OpenTibiaUnity.GameManager.GetFeature(GameFeature.GameClientPing))
                { // if there was no ping utility
                    if (_connectionWasLost)
                    {
                        _connectionWasLost = false;
                        onConnectionRecovered.Invoke();
